...
Menue::Menue()
{
	while (true)
	{
		...
		cout << "== CocktailMix | Operating Mode ==" << endl;   // first menu
...
void Menue::submenu_mix_it()
{
	while (true)   // submenu_1
	{
		unsigned int choice = 0;
		...
		if (choice > 0)
		{
			if (choice < counter)  
			{
				...
				if (choice <= cocktailSimulator->getCocktails().size())
				{
					...
					Cocktail *cocktail = cocktailSimulator->getCocktails()[choice - 1];
					cocktailSimulator->makeCocktail(cocktail);
				}
			}
			else
			{
